...
std::deque<NodeGraph::Node*> NodeGraph::findPath(Node* starting, Node* ending)
{
	...
	while (!openList.empty())
	{
		...
		if (currentNode == ending)
		{
			...
			while (currentNode->previous != nullptr)
			{
				...
				Node* laterNode = currentNode;
				currentNode = currentNode->previous;
				laterNode->previous = nullptr;
			}
			return path;
		}
		//end if statement
		...
		for (int i = 0; i < currentNode->connections.size(); i++)
		{
			...
			if (!bubbleSort(closedList, currentEdgeEnd))
			{
				...
				float currentHScore = updateScore(currentEdgeEnd, ending);
				...
				if (!bubbleSort(openList, currentEdgeEnd))
				{
...
bool NodeGraph::bubbleSort(std::deque<Node*> list, Node* lookingFor)
{
...
float NodeGraph::updateScore(Node* currentNode, Node* end)
{
